...
Second, how should we think about the factors that vary between different presentations? The work of \citeA{Kaminski2008} and much of the following work has focused on a single axis of concrete (or grounded) vs. abstract (or generic). A few authors have acknowledged that there are other features that may be irrelevant, such as decorative features like toppings on pizza slices \cite<e.g.>{Belenky2014}. However, is there really a single concrete vs. abstract axis that affects what is learned from a presentation, or is the space of possible presentations more complicated? Is ``concrete'' a broadly useful term for describing the features that differ between presentations, or is it just a proxy for other factors? Can presentations of a concept that connect to different types of knowledge result in different types of understanding? Some previous work would suggest that this can occur, at least with large differences in pedagogical strategy, such as learning by instruction vs. learning by doing \cite{Nokes2005}.  \par
