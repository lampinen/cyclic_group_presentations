...
We performed post-hoc logistic regressions predicting score on each which-representation question by the ratings of representation used. We found that neither modular nor polygon rating was significantly predictive of success on the inverse of zero questions ($\beta_{mod} = -0.02$, $z = -0.15$, $p = 0.88$; $\beta_{poly} = 0.25$, $z = 1.37$, $p = 0.17$). We suspect this may have been due to the fact that this was the third presentation of an inverse of zero question, and performance was very high in the hybrid group on this question. This is supported by the fact that the majority of the subjects got the question right, it had the only positive intercept of any of the which-representation regressions. Intriguigingly, we found that both modular and polygon rating were significantly predictive of success on inverse of non-zero questions ($\beta_{mod} = 1.09$, $z = 2.95$, $p = 0.003$; $\beta_{poly} = 1.11$, $z = 2.94$, $p = 0.003$). Both these findings seem suggestive of some integration occuring in the hybrid condition, such that the advantages of each representation are to some extent shared even when the other representation is used. 
\begin{figure}[H]
\centering
...
\end{subfigure}
\caption{Experiment 3 -- Which-representation response counts by score (inverse questions)}
\label{ex3_wr_in}
\end{figure}\noindent
...
\end{subfigure}
\caption{Experiment 3 -- Which-representation response counts by score (generator questions)}
...
The data from the which-representation questions, however, suggest that there was some integration occuring for the hybrid subjects by the end of the experiment (when they saw these questions), although it was far from perfect. This is corroborated by the 
%%TODO: elaborate
