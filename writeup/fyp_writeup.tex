...
It remains a question for future research how the choice of presentations what type of integration occurs. Hopefully this would shed light on how the hybrid presentation could be altered to encourage more uniform improvement across all question types and participants. Nevertheless, our results suggest that teaching multiple presentations may be beneficial to students' overall understanding. 
...
The reader might notice that some of the experimental groups in the work of Kaminski and colleagues in fact had hybrid-like elements. Specifically, some of the participants who saw the concrete presentations saw multiple distinct presentations (e.g. both fractional cups of liquid and fractional slices of pizza), and had their attention implicitly or explicitly drawn to the connections between the presentations. It is important to clarify that to be beneficial hybrid presentations must be constructed from distinct presentations with distinct advantages. Because of this the benefits will generally be increased the more distinct the presentations are, as long as they all contain useful information in an easily comprehensible format. The concrete presentations in the work of Kaminski et al. can be seen as drawing on the same numerical intuitions. We would expect that by combining both the concrete and generic presentations used by Kaminski and colleagues, one might be able to achieve better transfer performance more broadly (for example, seeing the generic presentation might allow participants to transfer to Kaminski and colleagues' original transfer task, while seeing the concrete presentations might give better ability to generalize to a new group order or a new concrete instantiation that shares the numerical structure of the taught concrete presentations).  
\subsection{Formalization \& Generalization}
None of the presentations seemed to encourage formal or general understanding particularly well, as evidenced by the low overall performance on the order $n$ questions. For example, despite the fact that performance on the inverse questions was around 75\% on average, only about one third of the participants were able to articulate the general formula for computing an inverse in cyclic group of unspecified order $n$. This may reflect the fact that this study was too short for participants understanding of specific groups to develop into a more general understanding, or it may be because the processes of reasoning explicitly or formally and the processes of reasoning implicitly and/or procedurally are not perfectly linked, and some effort is needed to go from one to the other \cite<e.g.>{Anderson1996,Reber1967,Davidson2012}. This may also explain why the advantages of the presentations didn't transfer to the order $n$ questions -- it may be difficult to extract the logic behind the subtraction algorithm for finding inverses in a way that can be made formal, and it may be difficult to use spatial intuitions about generators when thinking about all possible polygons. The process of learning to make this mapping between procedural and formal understanding may itself be a skill that is strengthened through exposure to mathematical reasoning. \par 
