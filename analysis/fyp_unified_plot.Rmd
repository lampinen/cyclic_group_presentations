...
plotting_data$question_type = factor(plotting_data$question_type)
plotting_data$question_type = factor(plotting_data$question_type,levels(plotting_data$question_type)[c(3,5,1,4,6,2)])
```

```{r}
ggplot(data=plotting_data,aes(question_type,percent_correct.mean,fill=condition)) + 
  geom_bar(stat="identity",position="dodge") +
  geom_errorbar(aes(ymax=percent_correct.mean+percent_correct.sem,ymin=percent_correct.mean-percent_correct.sem,width=0.5),position=position_dodge(width=0.9)) +
  scale_x_discrete(breaks=levels(plotting_data$question_type),
                      labels=c("Inverse of Non-zero (6)", "Inverse of Zero (6)", "Identifying Gen. (6)","Inverse of Non-zero (9)", "Inverse of Zero (9)", "Identifying Gen. (9)")) +  
  scale_fill_manual(values = c("#00BFC4", "#F8766D", "#FFCC33")) +
  scale_alpha_manual(values= c(0.33,1.0))+
