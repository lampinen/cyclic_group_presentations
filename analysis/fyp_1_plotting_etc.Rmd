...
We have reason to suspect there might be a difference for the generator true questions.
```{r}
# rt_mod = lm(rt ~ polygon,data=complete_data[grepl("generatorunder",gsub(" ","",complete_data$question)) & (grepl("hexagon",gsub(" ","",complete_data$question)) | grepl("+<sub>6",gsub(" ","",complete_data$question)) ) & gsub(" ","",correct_response) == 'Yes',])
rt_mod = lmer(rt ~ polygon+(1|subject),data=complete_data[grepl("generatorunder",gsub(" ","",complete_data$question)) & (grepl("nonagon",gsub(" ","",complete_data$question)) | grepl("+<sub>9",gsub(" ","",complete_data$question)) ) & (grepl('5',gsub(" ","",complete_data$question)) | grepl('2',gsub(" ","",complete_data$question))),])
summary(rt_mod) #what the heck

ggplot(data=complete_data[grepl("generatorunder",gsub(" ","",complete_data$question)) & (grepl("nonagon",gsub(" ","",complete_data$question)) | grepl("+<sub>9",gsub(" ","",complete_data$question)) ) & (grepl('5',gsub(" ","",complete_data$question)) | grepl('2',gsub(" ","",complete_data$question))),],aes(x=rt,fill=polygon)) +
  geom_histogram(position='dodge') +
  theme_bw()

#Get rid of the crazy outliers
tempstuff = complete_data[grepl("generatorunder",gsub(" ","",complete_data$question)) & (grepl("nonagon",gsub(" ","",complete_data$question)) | grepl("+<sub>9",gsub(" ","",complete_data$question)) ) & (grepl('5',gsub(" ","",complete_data$question)) | grepl('2',gsub(" ","",complete_data$question))),]
ggplot(data=tempstuff[tempstuff$rt < 2000,],aes(x=rt,fill=polygon)) +
  geom_histogram(position='dodge') +
  theme_bw()
```
...
complete_data$condition = factor(complete_data$condition,levels(complete_data$condition)[c(2,1)])
```

Effects of using diagram on Generator (True) in polygon case?:

```{r}
summarized_data = complete_data[grepl("generatorunder",gsub(" ","",complete_data$question)),] %>% filter(polygon) %>% mutate(group_order = ifelse((grepl("hexagon",gsub(" ","",question)) | grepl("+<sub>6",gsub(" ","",question))),'Order 6','Order 9')) %>% mutate(generator = ifelse(group_order=='Order 6',gsub(" ","",correct_response) == "Yes",(grepl("2",question) | grepl("3",question)))) %>% group_by(subject,condition,prompt,generator,group_order,orientation_history) %>% summarize(percent_correct = mean(score_strict)) %>% group_by(condition,generator,group_order,orientation_history) %>% summarize(percent_correct.mean = mean(percent_correct),percent_correct.sem = sem(percent_correct),n=n())

ggplot(data=summarized_data[summarized_data$generator,],aes(group_order,percent_correct.mean,fill=orientation_history)) +
  geom_bar(stat="identity",position="dodge") +
  geom_errorbar(aes(ymax=percent_correct.mean+percent_correct.sem,ymin=percent_correct.mean-percent_correct.sem,width=0.25),position=position_dodge(width=0.9)) +
#   annotate("text",x=1.0,y=1.02,label='*',alpha=0.5,size=8)+
#   annotate("text",x=2.0,y=1.02,label='**',alpha=0.5,size=8)+
#   annotate("path",x=c(0.75,0.75,1.25,1.25),y=c(0.98,1.0,1.0,0.98),alpha=0.5)+
#   annotate("path",x=c(1.75,1.75,2.25,2.25),y=c(0.98,1.0,1.0,0.98),alpha=0.5)+
  labs(y='% Correct', x='Order') + 
  scale_fill_manual(values = c("#00BFC4", "#F8766D")) +
  ggtitle("Generator (Generator = True, Polygon)")
```

```{r}
summarized_data = complete_data[grepl("generatorunder",gsub(" ","",complete_data$question)),] %>% filter(!polygon) %>% mutate(group_order = ifelse((grepl("hexagon",gsub(" ","",question)) | grepl("+<sub>6",gsub(" ","",question))),'Order 6','Order 9')) %>% mutate(generator = ifelse(group_order=='Order 6',gsub(" ","",correct_response) == "Yes",(grepl("2",question) | grepl("3",question)))) %>% group_by(subject,condition,prompt,generator,group_order) %>% summarize(percent_correct = mean(score_strict)) %>% group_by(condition,generator,group_order) %>% summarize(percent_correct.mean = mean(percent_correct),percent_correct.sem = sem(percent_correct))

ggplot(data=summarized_data[summarized_data$generator,],aes(group_order,percent_correct.mean)) +
  geom_bar(stat="identity",position="dodge") +
  geom_errorbar(aes(ymax=percent_correct.mean+percent_correct.sem,ymin=percent_correct.mean-percent_correct.sem,width=0.25),position=position_dodge(width=0.9)) +
#   annotate("text",x=1.0,y=1.02,label='*',alpha=0.5,size=8)+
#   annotate("text",x=2.0,y=1.02,label='**',alpha=0.5,size=8)+
...
#   annotate("path",x=c(1.75,1.75,2.25,2.25),y=c(0.98,1.0,1.0,0.98),alpha=0.5)+
  labs(y='% Correct', x='Order') + 
  scale_fill_manual(values = c("#00BFC4", "#F8766D")) +
  ggtitle("Generator (Generator = True, modular)")

```

Is group difference still present if those who used diagram are excluded
```{r}
summarized_data = complete_data[grepl("generatorunder",gsub(" ","",complete_data$question)),] %>% filter(!polygon | !orientation_history) %>% mutate(group_order = ifelse((grepl("hexagon",gsub(" ","",question)) | grepl("+<sub>6",gsub(" ","",question))),'Order 6','Order 9')) %>% mutate(generator = ifelse(group_order=='Order 6',gsub(" ","",correct_response) == "Yes",(grepl("2",question) | grepl("3",question)))) %>% group_by(subject,condition,prompt,generator,group_order) %>% summarize(percent_correct = mean(score_strict)) %>% group_by(condition,generator,group_order) %>% summarize(percent_correct.mean = mean(percent_correct),percent_correct.sem = sem(percent_correct))
...
#   annotate("path",x=c(0.75,0.75,1.25,1.25),y=c(0.98,1.0,1.0,0.98),alpha=0.5)+
#   annotate("path",x=c(1.75,1.75,2.25,2.25),y=c(0.98,1.0,1.0,0.98),alpha=0.5)+
  labs(y='% Correct', x='Diagram Use') + 
  scale_fill_manual(values = c("#00BFC4", "#F8766D")) +
  ggtitle("Generator (Generator = True, Polygon)")
```

It would appear that 1) Subjects in the polygon group who interacted with the diagram performed better, though only a few did so, and 2) this was not the sole source of the positive effect of the polygon group. It might be, however, that some subjects just pointed at the diagram on their screen, for example, we only know whether they actually clicked to move the arrow around.
...
ggplot(data=summarized_data,aes(condition,percent_correct.mean,fill=condition)) +
  ...
  annotate("text",x=1.5,y=1.04,label='ns',alpha=0.5,size=8) +
  labs(y='% Correct') +
  scale_fill_manual(values = c("#00BFC4", "#F8766D")) +
  ggtitle("Operation Order 6 (Learning)")
```
...
ggplot(data=summarized_data,aes(condition,percent_correct.mean,fill=condition)) +
  ...
  labs(y='% Correct') + 
  scale_fill_manual(values = c("#00BFC4", "#F8766D")) +
  ggtitle("Operation Order 9 (Testing)")
```
...
ggplot(data=summarized_data,aes(inverse_type,percent_correct.mean,fill=condition)) +
  ...
  annotate("text",x=1,y=1.04,label='ns',alpha=0.5,size=8)+
  annotate("path",x=c(0.75,0.75,1.25,1.25),y=c(0.98,1.0,1.0,0.98),alpha=0.5)+
  annotate("text",x=2,y=1.01,label='***',alpha=0.5,size=8)+
  annotate("path",x=c(1.75,1.75,2.25,2.25),y=c(0.98,1.0,1.0,0.98),alpha=0.5)+
  ggtitle("Inverse Order 6 (Learning) Results")
```


Inverse test:
```{r}
summarized_data = complete_data[grepl("Whatistheinverse",gsub(" ","",complete_data$question)) & !grepl("Remember,theinverse",gsub(" ","",complete_data$question)),] %>% group_by(subject,condition,prompt) %>% summarize(percent_correct = mean(score_strict)) %>% group_by(condition) %>% summarize(percent_correct.mean = mean(percent_correct),percent_correct.sem = sem(percent_correct))
...
ggplot(data=summarized_data,aes(condition,percent_correct.mean,fill=condition)) +
  ...
  annotate("text",x=1.5,y=1.01,label='***',alpha=0.5,size=8)+
  labs(y='% Correct') + 
  scale_fill_manual(values = c("#00BFC4", "#F8766D")) +
  ggtitle("Inverse of Zero")
```
...
summarized_data = complete_data[grepl("formula",gsub(" ","",complete_data$question)),] %>% group_by(subject,condition,prompt) %>% summarize(percent_correct = mean(score_strict)) %>% group_by(condition) %>% summarize(percent_correct.mean = mean(percent_correct),percent_correct.sem = sem(percent_correct))

ggplot(data=summarized_data,aes(condition,percent_correct.mean,fill=condition)) +
  geom_bar(stat="identity",position="dodge",width=0.5) +
  geom_errorbar(aes(ymax=percent_correct.mean+percent_correct.sem,ymin=percent_correct.mean-percent_correct.sem),position=position_dodge(),width=0.25) +
  annotate("text",x=1.5,y=1.02,label='*',alpha=0.5,size=8)+
  annotate("path",x=c(1.0,1.0,2.0,2.0),y=c(0.98,1.0,1.0,0.98),alpha=0.5)+
  scale_fill_manual(values = c("#00BFC4", "#F8766D")) +
  labs(y='% Correct') + 
  ggtitle("Inverse Formula Results")
...
ggplot(data=summarized_data,aes(generator,percent_correct.mean,fill=condition)) +
  geom_bar(stat="identity",position="dodge") +
  geom_errorbar(aes(ymax=percent_correct.mean+percent_correct.sem,ymin=percent_correct.mean-percent_correct.sem),position="dodge") +
  annotate("text",x=2.0,y=1.0,label='p = 0.98',alpha=0.5)+
  annotate("text",x=1.0,y=1.0,label='p = 0.046',alpha=0.5)+
  ggtitle("Generator Order 6 (Learning) Results")
```
...
```{r}
summarized_data = complete_data[grepl("generatorunder",gsub(" ","",complete_data$question)) & (grepl("hexagon",gsub(" ","",complete_data$question)) | grepl("+<sub>6",gsub(" ","",complete_data$question)) | grepl("nonagon",gsub(" ","",complete_data$question)) | grepl("+<sub>9",gsub(" ","",complete_data$question))),] %>% mutate(group_order = ifelse((grepl("hexagon",gsub(" ","",question)) | grepl("+<sub>6",gsub(" ","",question))),'Order 6','Order 9')) %>% mutate(generator = ifelse(group_order=='Order 6',gsub(" ","",correct_response) == "Yes",(grepl("2",question) | grepl("5",question)))) %>% group_by(subject,condition,prompt,generator,group_order) %>% summarize(percent_correct = mean(score_strict)) %>% group_by(condition,generator,group_order) %>% summarize(percent_correct.mean = mean(percent_correct),percent_correct.sem = sem(percent_correct))
...
ggplot(data=summarized_data[!summarized_data$generator,],aes(group_order,percent_correct.mean,fill=condition)) +
  geom_bar(stat="identity",position="dodge") +
  geom_errorbar(aes(ymax=percent_correct.mean+percent_correct.sem,ymin=percent_correct.mean-percent_correct.sem,width=0.25),position=position_dodge(width=0.9)) +
  annotate("text",x=1.0,y=1.04,label='ns',alpha=0.5,size=8)+
  annotate("text",x=2.0,y=1.02,label='*',alpha=0.5,size=8)+
  annotate("path",x=c(0.75,0.75,1.25,1.25),y=c(0.98,1.0,1.0,0.98),alpha=0.5)+
  annotate("path",x=c(1.75,1.75,2.25,2.25),y=c(0.98,1.0,1.0,0.98),alpha=0.5)+
  labs(y='% Correct', x='Group Order') + 
  scale_fill_manual(values = c("#00BFC4", "#F8766D")) +
  ggtitle("Generator (Generator = False)")
```
...
```{r}
summarized_data = complete_data[grepl("Trueorfalse",gsub(" ","",complete_data$question)),] %>% mutate(tf_Q= ifelse(grepl("evennumber",gsub(" ","",question)),"Q1",ifelse(grepl("oddnumber",gsub(" ","",question)),"Q2",ifelse(grepl("divide",gsub(" ","",question)),"Q3","Q4")))) %>% group_by(subject,condition,tf_Q) %>% summarize(percent_correct = mean(score_strict)) %>% group_by(condition,tf_Q) %>% summarize(percent_correct.mean = mean(percent_correct),percent_correct.sem = sem(percent_correct))

ggplot(data=summarized_data,aes(tf_Q,percent_correct.mean,fill=condition)) +
  geom_bar(stat="identity",position="dodge",width=0.5) +
  geom_errorbar(aes(ymax=percent_correct.mean+percent_correct.sem,ymin=percent_correct.mean-percent_correct.sem,width=0.25),position=position_dodge(width=0.5)) +
  scale_fill_manual(values = c("#00BFC4", "#F8766D")) +
  labs(y = '% Correct',x="Question") + 
  ylim(0,1)+
  ggtitle("Generator T/F Results by Question")
```
...
```{r}
summarized_data = complete_data[grepl("always,sometimes,ornever",gsub(" ","",complete_data$question)),] %>% group_by(subject,condition,prompt) %>% summarize(percent_correct = mean(score_strict)) %>% group_by(condition) %>% summarize(percent_correct.mean = mean(percent_correct),percent_correct.sem = sem(percent_correct))

ggplot(data=summarized_data,aes(condition,percent_correct.mean,fill=condition)) +
  geom_bar(stat="identity",position="dodge",width=0.5) +
  geom_errorbar(aes(ymax=percent_correct.mean+percent_correct.sem,ymin=percent_correct.mean-percent_correct.sem,width=0.125),position="dodge") +
  annotate("text",x=1.5,y=1.04,label='ns',alpha=0.0,size=8)+
  scale_fill_manual(values = c("#00BFC4", "#F8766D")) +
  labs(y='% Correct', x='Condition') + 
  ggtitle("Generator A/S/N Results")
```

```{r}
summarized_data = complete_data[grepl("always,sometimes,ornever",gsub(" ","",complete_data$question)),] %>% mutate(asn_Q=ifelse(grepl("inverse",gsub(" ","",question)),"Q1",ifelse(grepl("notagenerator",gsub(" ","",question)),"Q3","Q2"))) %>% group_by(subject,condition,asn_Q) %>% summarize(percent_correct = mean(score_strict)) %>% group_by(condition,asn_Q) %>% summarize(percent_correct.mean = mean(percent_correct),percent_correct.sem = sem(percent_correct))

ggplot(data=summarized_data,aes(asn_Q,percent_correct.mean,fill=condition)) +
  geom_bar(stat="identity",position="dodge",width=0.5) +
  geom_errorbar(aes(ymax=percent_correct.mean+percent_correct.sem,ymin=percent_correct.mean-percent_correct.sem,width=0.25),position=position_dodge(width=0.5)) +
  scale_fill_manual(values = c("#00BFC4", "#F8766D")) +
  labs(y = '% Correct',x="Question") + 
  ylim(0,1)+
  ggtitle("Generator A/S/N Results by Question")
```
