...
for (i in 1:length(files)) {
  ...
  for (trial_i in 1:length(these_data[1][[1]])) {
    if(grepl("question",gsub(" ","",these_data[1][[1]][[trial_i]]$trial_type))) {
      ...
      if (grepl('polygon',this_condition)) {
        orientation_history[j] = length(these_data[1][[1]][[trial_i]]$orientation_history) > 1 #1 at beginning because of initial orientation
      }
      else {
        orientation_history[j] = NA
      }
...
------------------------
```{r}
complete_data = data.frame(subject=factor(subject),condition=condition,question=prompt,correct_response=correct_response,response=response,rt=rt,trial_index=trial_index,score=score,orientation_history=orientation_history) 
...
ggplot(data=summarized_data,aes(condition,percent_correct.mean,fill=condition)) +
  geom_bar(stat="identity",position="dodge") +
  geom_errorbar(aes(ymax=percent_correct.mean+percent_correct.sem,ymin=percent_correct.mean-percent_correct.sem),position="dodge") +
  annotate("text",x=1.5,y=1.0,label='*',alpha=0.5,width=8)+
  labs(y='% Correct') + 
...
ggplot(data=summarized_data,aes(condition,percent_correct.mean,fill=condition)) +
  geom_bar(stat="identity",position="dodge") +
  geom_errorbar(aes(ymax=percent_correct.mean+percent_correct.sem,ymin=percent_correct.mean-percent_correct.sem),position="dodge") +
  annotate("text",x=1.5,y=1.0,label='p = 0.49',alpha=0.5)+
