...
write.csv(stuff,'fyp_3_selected_bootstrap_estimates.csv')
write.csv(bootstrapped_coefficients,'fyp_3_full_bootstrap_coefficients.csv')
```
...
polygon_coefficients = summary(mod)$coefficients[c(15,29:40),1]+modular_coefficients
...
print(paste("theta: ",theta,", phi: ",phi,', LL: ',log_likelihood_of_full_data(hierarchical_data,theta,phi),', BIC: ', log(100)*1-2*log_likelihood_of_full_data(hierarchical_data,theta,phi)))
...
print(paste("theta: ",theta,", phi: ",phi,', LL: ',log_likelihood_of_full_data(hierarchical_data,theta,phi),', BIC: ', log(100)*2-2*log_likelihood_of_full_data(hierarchical_data,theta,phi)))
...
print(paste("theta: ",theta,", phi: ",phi,', BIC: ', 2*log(100)-2*log_likelihood_of_full_data(hierarchical_6_data,theta,phi)))
...
print(paste("theta: ",theta,", phi: ",phi,', LL: ', log_likelihood_of_full_data(hierarchical_9_data,theta,phi), ', BIC (including order n):', 2*log(100)-2*log_likelihood_of_full_data(hierarchical_not6_data,theta,phi)))
...
whichrep_data$whichrep_polygon = factor(whichrep_data$whichrep_polygon)
whichrep_data$whichrep_modular = factor(whichrep_data$whichrep_modular)
...
whichrep_data = whichrep_data %>% mutate(correct=ifelse(score==1,"Correct","Incorrect")) %>% mutate(correct=factor(correct))
```
```{r}
temp_data = whichrep_data[!is.na(whichrep_data$whichrep_polygon) & grepl('inverseof0',gsub(' ','',whichrep_data$question)),] %>% group_by(correct,whichrep_polygon,whichrep_modular) %>% summarize(count = n()) %>% complete(whichrep_polygon,whichrep_modular,correct,fill=list(count=0))
ggplot(data=temp_data,aes(x=whichrep_polygon,y=whichrep_modular)) +
  geom_tile(aes(fill=count)) +
  scale_fill_continuous(low="#FFFFFF",high="#FF3333") +
  geom_text(aes(label = ifelse(count > 0,count,"")),size=12) +
  labs(x = "Polygon Rating",y="Modular Rating",title="Inverse of Zero: Which Representation?")+
  facet_grid(.~correct,labeller=label_value)+ 
  theme_bw(base_size=20)+theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())
...
```{r}
temp_data = whichrep_data[!is.na(whichrep_data$whichrep_polygon) & grepl('inverseof4',gsub(' ','',whichrep_data$question)),] %>% group_by(correct,whichrep_polygon,whichrep_modular) %>% summarize(count = n()) %>% complete(whichrep_polygon,whichrep_modular,correct,fill=list(count=0))
ggplot(data=temp_data,aes(x=whichrep_polygon,y=whichrep_modular)) +
  geom_tile(aes(fill=count)) +
  scale_fill_continuous(low="#FFFFFF",high="#FF3333") +
  geom_text(aes(label = ifelse(count > 0,count,"")),size=12) +
  labs(x = "Polygon Rating",y="Modular Rating",title="Inverse of Non-zero: Which Representation?")+
  facet_grid(.~correct,labeller=label_value)+ 
  theme_bw(base_size=20)+theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())
#
...
temp_data = whichrep_data[!is.na(whichrep_data$whichrep_polygon) & grepl('4agenerator',gsub(' ','',whichrep_data$question)),] %>% group_by(correct,whichrep_polygon,whichrep_modular) %>% summarize(count = n()) %>% complete(whichrep_polygon,whichrep_modular,correct,fill=list(count=0))
ggplot(data=temp_data,aes(x=whichrep_polygon,y=whichrep_modular)) +
  geom_tile(aes(fill=count)) +
  scale_fill_continuous(low="#FFFFFF",high="#FF3333") +
  geom_text(aes(label = ifelse(count > 0,count,"")),size=12) +
  labs(x = "Polygon Rating",y="Modular Rating",title="Identifying Generators: Which Representation?")+
  facet_grid(.~correct,labeller=label_value)+ 
  theme_bw(base_size=20)+theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())
...
temp_data = whichrep_data[!is.na(whichrep_data$whichrep_polygon) & grepl('Trueorfalse',gsub(' ','',whichrep_data$question)),] %>% group_by(correct,whichrep_polygon,whichrep_modular) %>% summarize(count = n()) %>% complete(whichrep_polygon,whichrep_modular,correct,fill=list(count=0))
ggplot(data=temp_data,aes(x=whichrep_polygon,y=whichrep_modular)) +
  geom_tile(aes(fill=count)) +
  scale_fill_continuous(low="#FFFFFF",high="#FF3333") +
  geom_text(aes(label = ifelse(count > 0,count,"")),size=12) +
  labs(x = "Polygon Rating",y="Modular Rating",title="True/False: Which Representation?")+
  facet_grid(.~correct,labeller=label_value)+ 
  theme_bw(base_size=20)+theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank())
...
Trying to combine this with the hierarchical:
```{r}
temp = augmented_data %>% group_by(subject,theta,phi) %>% summarize(weight = head(weight,1)) %>% mutate(estimated_strategy = ifelse(theta,"integrate",ifelse(phi,"polygon","modular"))) %>% group_by(subject)  %>% top_n(n=1,wt=weight) %>% select(subject,estimated_strategy) #MLE estimates of strategy used by each subject from the hierarchical modeling
temp$estimated_strategy = factor(temp$estimated_strategy)
whichrep_data = whichrep_data %>% inner_join(temp) 
```

```{r}
ggplot(data=whichrep_data[!is.na(whichrep_data$whichrep_polygon) & grepl('inverseof0',gsub(' ','',whichrep_data$question)),],aes(x=whichrep_polygon,y=whichrep_modular,color=estimated_strategy)) +
  geom_point(position=position_jitter()) +
  theme_bw()

ggplot(data=whichrep_data[!is.na(whichrep_data$whichrep_polygon) & grepl('inverseof4',gsub(' ','',whichrep_data$question)),],aes(x=whichrep_polygon,y=whichrep_modular,color=estimated_strategy)) +
  geom_point(position=position_jitter()) +
  theme_bw()


ggplot(data=whichrep_data[!is.na(whichrep_data$whichrep_polygon) & grepl('4agenerator',gsub(' ','',whichrep_data$question)),],aes(x=whichrep_polygon,y=whichrep_modular,color=estimated_strategy)) +
  geom_point(position=position_jitter()) +
  theme_bw()

ggplot(data=whichrep_data[!is.na(whichrep_data$whichrep_polygon) & grepl('Trueorfalse',gsub(' ','',whichrep_data$question)),],aes(x=whichrep_polygon,y=whichrep_modular,color=estimated_strategy)) +
  geom_point(position=position_jitter()) +
  theme_bw()
...
Correlations
```{r}
q = whichrep_data %>% filter(is.na(explain)) %>% mutate(estimated_integrate = 1*(estimated_strategy=='integrate'),estimated_polygon = 1*(estimated_strategy=='polygon'),estimated_modular = 1*(estimated_strategy=='modular'))

cor.test(q$whichrep_modular,q$estimated_modular)
cor.test(q$whichrep_polygon,q$estimated_modular)
cor.test(q$whichrep_polygon,q$estimated_polygon)
cor.test(q$whichrep_modular,q$estimated_polygon)
cor.test(q$whichrep_polygon,q$estimated_integrate)
cor.test(q$whichrep_modular,q$estimated_integrate)
```


Other tests with hiearchical data
```{r}
q = model_data %>% inner_join(temp) %>% mutate(estimated_integrate = 1*(estimated_strategy=='integrate'))
cor.test(q$score_strict,q$estimated_integrate)

q = complete_data %>% filter(condition == 'hybrid' & grepl('formula',gsub(' ','',complete_data$question))) %>% inner_join(temp)
qmod = lm(data=q,score_strict ~ estimated_strategy)
```

```{r}
wr_mod_in0 = glm(score ~ (whichrep_modular + whichrep_polygon)*estimated_strategy,data=whichrep_data[!is.na(whichrep_data$whichrep_modular) & grepl('inverseof0',gsub(' ','',whichrep_data$question)),],family='binomial')
summary(wr_mod_in0)

wr_mod_innz = glm(score ~ (whichrep_modular + whichrep_polygon)*estimated_strategy,data=whichrep_data[!is.na(whichrep_data$whichrep_modular) & grepl('inverseof4',gsub(' ','',whichrep_data$question)),],family='binomial')
summary(wr_mod_innz)

wr_mod_gen = glm(score ~ (whichrep_modular + whichrep_polygon)*estimated_strategy,data=whichrep_data[!is.na(whichrep_data$whichrep_modular) & grepl('4agenerator',gsub(' ','',whichrep_data$question)),],family='binomial')
summary(wr_mod_gen)


wr_mod_gen_TF = glm(score ~ (whichrep_modular + whichrep_polygon)*estimated_strategy,data=whichrep_data[!is.na(whichrep_data$whichrep_modular) & grepl('Trueorfalse',gsub(' ','',whichrep_data$question)),],family='binomial')
summary(wr_mod_gen_TF)
```
...
gt_data = model_data %>% filter(grepl('generator_true',model_data$question_type))
gt_mod = glmer(data=gt_data[gt_data$condition == 'polygon',],family=binomial,score ~ used_diagram+(1|subject))
summary(gt_mod)
gt_mod = glmer(data=gt_data[gt_data$condition == 'polygon',],family=binomial,score ~ used_diagram+rt+(1|subject))
summary(gt_mod)
...
Yes, moreso than the overall engagement, though both were quite large effects
