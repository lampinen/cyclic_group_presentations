...
bootstrap_delta_mean_95CI = function(data_1,data_2,num_iterations=1000) {
  results = rep(0,num_iterations)
  for (i in 1:num_iterations) {
    n1 = length(data_1)
    n2 = length(data_2)
    results[i] = mean(sample(data_1,n1,replace=TRUE))-mean(sample(data_2,n2,replace=TRUE))
  }
...
for (q_i in 1:length(questions)) {
  print(questions[q_i])
  boot_delta_means = bootstrap_delta_mean_95CI(model_data[model_data$question_type == questions[q_i] & model_data$polygon,]$score_strict,model_data[model_data$question_type == questions[q_i] & !model_data$polygon,]$score_strict)
...
```{r}
model_data = model_data %>% mutate(polygon_word_count = str_count(response,'move') + str_count(response,'spaces') + str_count(response,'clockwise') + str_count(response,'steps') + str_count(response,'count') + str_count(response,'arrow') + str_count(response,'hexagon') + str_count(response,'nonagon') + str_count(response,'ngon') + str_count(response,'spots') + str_count(response,'positions'),modular_word_count = str_count(response,'add') + str_count(response,'subtract') + str_count(response,'plus') + str_count(response,'minus') + str_count(response,'[+]')) %>% mutate(polygon_wc_higher = polygon_word_count > modular_word_count)
```

```{r}
xtabs(~polygon+polygon_wc_higher,data=model_data[grepl('explain',gsub(' ','',model_data$question)),])
xtabs(~polygon+polygon_wc_higher,data=model_data[grepl('explain',gsub(' ','',model_data$question)) & model_data$question_type=='inverse_nonzero_9',])
xtabs(~polygon+polygon_wc_higher,data=model_data[grepl('explain',gsub(' ','',model_data$question)) & model_data$question_type=='generator_true_9',])
```
